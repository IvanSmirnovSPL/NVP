...
void cir_flux(double left_params[M], double right_params[M], double dt, double dx, double flux[M])
{
    ...
    for (int i = 0; i < M; i++)
    {
        ...
        flux[i] += - 0.5 * (dx / dt) * (right_params[i] - left_params[i]);
    }
...
void cir_flux_( double left_params[M], double right_params[M], double flux[M] ) {
