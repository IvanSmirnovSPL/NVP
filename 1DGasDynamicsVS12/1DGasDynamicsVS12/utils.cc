...
void calc_flux( double u_cons_l[M], double u_cons_r[M], double dt, double dx, double flux[M] ) {

    int scheme = SCHEME;                            /* расчетная схема */

    switch ( scheme ) {
        case CIR:
            cir_flux( u_cons_l, u_cons_r, dt, dx, flux );
            // cir_flux_(u_cons_l, u_cons_r, flux);
            break;
        default:
...
void init_solution( double u_cons[N][M] ) {
